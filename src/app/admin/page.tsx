...
export default async function AdminOverview() {
  const [
    ...
    () =>
      [
        ...
        0,
        0,
        { _sum: { btcHoldings: null } },
        { _sum: { totalInvested: null } },
      ] as const
  );
  ...
  const cells = [
    ...
    ['Total BTC Held', btcAgg._sum.btcHoldings?.toString() ?? '0'],
    [
      'Total Contributions',
      `$${investedAgg._sum.totalInvested?.toString() ?? '0'}`,
    ],
  ] as const;
      ...
      >
        {cells.map(([label, value]) => (
          ...
          >
            ...
            <div
              className="font-display font-semibold text-xl mt-1.5"
              style={{ fontFamily: 'Space Grotesk' }}
            >
              {value}
            </div>
