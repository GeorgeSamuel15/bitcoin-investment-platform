  ...
  useEffect(() => {
    Promise.all([
      ...
      const combined: Tx[] = [
        ...
        ...(orders as InvestmentOrder[]).map((o) => ({
          type: 'investment',
          id: o.id,
          amountUsd: Number(o.amountUsd),
          btcAmount: Number(o.estimatedBtc),
          status: o.status,
          date: o.createdAt,
        })),
        ...
        {['all', 'deposit', 'investment', 'withdrawal'].map((f) => (
          <button
            ...
            style={{
              ...
              background:
                f === filter ? 'var(--orange)' : 'transparent',
              color:
                f === filter ? '#1B1000' : 'var(--muted)',
            }}
          >
            {f}
          </button>
      ...
      <table className="w-full text-sm">
        <thead>
          <tr style={{ borderBottom: '1px solid var(--line)' }}>
            {['Type', 'ID', 'USD', 'BTC', 'Status', 'Date'].map(
              (h) => (
                <th
                  key={h}
                  className="text-left py-2.5 text-xs font-mono uppercase"
